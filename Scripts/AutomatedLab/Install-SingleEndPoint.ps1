...
Invoke-LabCommand -ComputerName (Get-LabVM) -ActivityName "Cleanup" -ScriptBlock  {
    ...
    if (Test-Path -Path 'edgeremoval.ps1' ){
        Remove-Item 'edgeremoval.ps1' -Recurse -Force
    }
}
...
# Reboot
$LabClients = Get-LabVM | Where-Object {( $_.OperatingSystem -NotLike "*Windows Server*" -and $_.OperatingSystemType -eq "Windows" )}
