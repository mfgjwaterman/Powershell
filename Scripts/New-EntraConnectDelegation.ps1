...
<#
    ...
    .EXAMPLE
    New-EntraConnectDelegation.ps1
    

    .EXAMPLE
    New-EntraConnectDelegation.ps1 -CreateDefaultGroups -OU "OU=Groups,OU=Tier 0,OU=Admin,DC=sandbox,DC=lab"
    Creates default groups in a designated Organizational Unit.

    .EXAMPLE
    New-EntraConnectDelegation.ps1 -PasswordHashSync -SecurityGroup "MSEC - Password Hash Sync"
    Delegates the Password Hash Sync funtionality to the given security group.

    .EXAMPLE
    New-EntraConnectDelegation.ps1 -EntraConnectFeature 'Group Writeback' -OU OU=test,DC=sandbox,DC=lab -SecurityGroup "MSEC - Group Writeback"
